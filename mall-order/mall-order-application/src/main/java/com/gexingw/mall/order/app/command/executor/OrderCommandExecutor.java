package com.gexingw.mall.order.app.command.executor;
...
public class OrderCommandExecutor {
    ...
    public Long add(OrderAddCommand addCommand) {
        // 配送地址
        OrderAddCommand.Shipping shipping = addCommand.getShipping();
        OrderShippingAddress shippingAddress = shippingAddressGateway.getById(shipping.getAddressId());
        if (shippingAddress == null) {
            throw new BizNotFoundException(RespCode.NOT_FOUND, "配送地址不存在！");
        }

        // 查询操作人信息
        User user = userGateway.getById(1L);
        if (user == null) {
            throw new BizNotFoundException(RespCode.NOT_FOUND, "用户信息不存在！");
        }
        //noinspection unused
    ...
    public boolean delete(Long id) {
        ...
        if (!orderGateway.exist(id)) {
            throw new BizNotFoundException(RespCode.NOT_FOUND, "订单不存在！");
        }
